...
export const HELP_GROUPS: HelpGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'oosRegimeCollect', term: 'Collect OOS by Vol Regime', body: 'Out-of-sample testing, organized by volatility state. Pick a lens (Exp/Con · +Stable · Term) and a regime — the dashboard lists every date window that regime occurred (vol clusters, so they\'re multi-day runs) and prefills the start date. Replay/collect those exact sessions with Gunship Lite into your OOS bucket; each trade auto-tags to its regime by date (prior session, no lookahead). Then "By Vol Regime" compares the in-sample profile vs this collected OOS — does the edge hold per regime?' },
      { id: 'regimeExpanding', term: 'Expanding (vol building)', body: 'Volatility is RISING — VVIX momentum is above its trailing-year median, so fear/uncertainty is accelerating. Breakouts are a long-volatility payoff: in expanding vol moves tend to RUN FARTHER (bigger MFE) but also whip harder against you first (bigger MAE), and this is where account-busting losing streaks cluster. Risk read: the move can pay more, but drawdowns deepen — size DOWN (vol-target) and let winners run / give the stop room, rather than adding size.' },
      { id: 'regimeContracting', term: 'Contracting (vol calming)', body: 'Volatility is FALLING / low — VVIX momentum below its trailing-year median, ranges tightening, fear draining out. Breakouts follow through less and false breakouts are more common; both MFE and MAE compress. Risk read: tighter, more realistic targets and be more selective — the big runners are rarer here. The calmest regime to grind, but don\'t expect the fat tails that pay in Expanding.' },
      { id: 'regimeStable', term: 'Stable (neutral momentum)', body: 'VVIX momentum sits in the middle band (40th–60th percentile) — vol neither clearly building nor calming, often a transition/quiet patch. It\'s a narrow band, so it has the fewest days and the thinnest samples; treat it as directional and lean on the 2-state (Expanding/Contracting) view for decisions.' },
      { id: 'regimeContango', term: 'Contango (normal curve)', body: 'The VIX term structure is upward-sloping — near-term implied vol (VIX9D) is BELOW 30-day (VIX). This is the market\'s default state (~80% of days): calm, no acute near-term fear. The benign regime for most setups.' },
      { id: 'regimeBackwardation', term: 'Backwardation (inverted curve)', body: 'The VIX term structure is INVERTED — near-term vol (VIX9D) spikes ABOVE 30-day (VIX), meaning the market fears the next few days more than the next month. Acute stress (~20% of days); tail events and crashes cluster here. The dangerous regime — the one to size down or stand aside in unless the move specifically earns its keep here out-of-sample.' },
